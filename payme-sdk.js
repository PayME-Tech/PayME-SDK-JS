class PaymeWebSdk {
  constructor(configs, settings) {
    ...
    this.env = configs.dataInit.appEnv.toLowerCase() || 'production'
    this.id = settings.id
    this.dimension = {
  ...
  createIfrm(configs) {
    ...
    let link = ''
    if (this.env === 'dev') link = 'http://localhost:3000'
    else if (this.env === 'sandbox') link = 'https://sbx-sdk.payme.com.vn'
